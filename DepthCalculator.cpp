...
std::vector<std::vector<double> > DepthCalculator::getInflectionPoints( std::vector<std::vector<double> > depthsAndTempsFull) {
	...
	Tk1,          //Temp  1
	Tk2,          //Temp  2
	accelerationPrevious=0,  //Previous slope prime
	accelerationCurrent=0,   //Current slope prime
	mag;
	...
	depthsAndTempsFiltered=depthsAndTempsFull;
...
//	}
	...
	for (unsigned int k = 0; k < depthsAndTempsFiltered.size() - 2; k++) {
		...
		Dk0 = depthsAndTempsFiltered[ k + 0 ][ 0 ],
		Dk1 = depthsAndTempsFiltered[ k + 1 ][ 0 ],
		Dk2 = depthsAndTempsFiltered[ k + 2 ][ 0 ],
		Tk0 = depthsAndTempsFiltered[ k + 0 ][ 1 ],
		Tk1 = depthsAndTempsFiltered[ k + 1 ][ 1 ],
		Tk2 = depthsAndTempsFiltered[ k + 2 ][ 1 ];



		if( ( Tk0 - Tk1 ) *  ( Tk1 - Tk2 ) !=0 ){

			accelerationPrevious = ( Dk0 - Dk1 )/( Tk0 - Tk1 )*( Tk0 - Tk1 ) -
					    ( Dk1 - Dk2 )/( Tk0 - Tk1 )*( Tk1 - Tk2 );

			mag = std::abs( std::abs( Tk0 ) - std::abs( Tk1 ) );

			if (accelerationPrevious * accelerationCurrent < 0 ){

//				infPoint.push_back((Dk0+Dk1+Dk2)/3.0);
//				infPoint.push_back((Tk0+Tk1+Tk2)/3.0);

//				infPoint.push_back(Dk0);
//				infPoint.push_back(Tk0);
				infPoint.push_back(Dk1);
				infPoint.push_back(Tk1);
//				infPoint.push_back(Dk2);
//				infPoint.push_back(Tk2);
				...
				if (iSize > 1) {
					...
					int p0 = (int) std::round(infPoints[iSize - 1][1]);
					int p1 = (int) std::round(infPoints[iSize - 2][1]);
			...
			accelerationCurrent = accelerationPrevious;
		}
